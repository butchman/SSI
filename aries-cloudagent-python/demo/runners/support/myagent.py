...
class MyAgent:
    ...
    def add_task(self,cmd,id,data):
        self.tasklist.append(atask(cmd,id,data))
    ...
    async def register_schema_and_creddef(
        ...
        with open('schema.txt', 'w') as json_file1:
            json.dump(schema_body, json_file1)
                ...
                if handler == 'api_handle_issuecred':
                    ...
                    if self.invitaion_url['invitation'] and self.invitaion_url['invitation']['recipientKeys']:
                        if len(self.invitaion_url['invitation']['recipientKeys']) >= 0:
                            self.add_task(handler, str(self.invitaion_url['invitation']['recipientKeys'][0]), payload)
                            log_msg('New task(1)|' + str(handler) + "|" + str(self.invitaion_url['invitation']['recipientKeys'][0]) +"|"+ str(payload))
                        else:
                            self.add_task(handler, str(self.invitaion_url['invitation']['recipientKeys']), payload)
                            log_msg('New task(2)|' + str(handler) + "|" + str(
                                self.invitaion_url['invitation']['recipientKeys']) + "|" + str(payload))
                    elif self.invitaion_url['connection_id']:
                        self.add_task(handler, str(self.invitaion_url['connection_id']), payload)
                        log_msg('New task(3)|' + str(handler) + "|" + str(self.invitaion_url['connection_id']) + "|" + str(payload))
