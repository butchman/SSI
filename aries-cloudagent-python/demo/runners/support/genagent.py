...
class atask(object):
    def __init__(self, cmd, id, data, conn_id):
...
class GenAgent:
    def __init__(
        ...
        **params,
        ...
        if params.get("internal_host"):
            self.internal_host = params.get("internal_host")
        else:
            self.internal_host = internal_host or DEFAULT_INTERNAL_HOST
        if params.get("external_host"):
            self.external_host = params.get("external_host")
        else:
        ...
        self.params = params
        self.proc = None
        self.myapi_port = None
        self.myapi_url = None
        self.client_session: ClientSession = ClientSession()
        self.webapp_session: ClientSession = ClientSession()
        ...
        if params.get("wallet_seed"):
            self.seed = params.get("wallet_seed")
        ...
        if params.get("invitaion_url"):
            self.invitaion_url = params.get("invitaion_url")
        else:
            self.invitaion_url = ''
    ...
    def add_task(self,cmd,id,data,conn_id):
        self.tasklist.append(atask(cmd,id,data,conn_id))

    def find_key(self, id):
        for i in range(len(self.tasklist)):
            print('find_key|'+str(self.tasklist[i].id)+'|'+str(id))
            if self.tasklist[i].id == id:
                print('find_key-FOUND|' + str(self.tasklist[i].id) + '|' + str(id))
                return i

        return -1

    def find_cred(self, id):
        for i in range(len(self.active_schema_list)):
            print('find_cred|'+str(self.active_schema_list[i]['cred_id'])+'|'+str(id))
            if (self.active_schema_list[i]['cred_id'] == id) or (self.active_schema_list[i]['cred_alias'] == id) or (self.active_schema_list[i]['cred_name'] == id):
                print('find_cred-FOUND|' + str(self.active_schema_list[i]['cred_id']) + '|' + str(id))
                return i
    ...
    async def register_schema_and_creddef(
        ...
        cred_def_tag = (self.ident + "|" + schema_name)
    ...
    async def fetchme(self,session, url):
    ...
    async def send_webapp_data(self,url):
        #data["did"] = 'did'
        #data["verkey"] = 'verkey'

        #async with self.webapp_session.post("http://127.0.0.1:3000/register", json=data) as resp:
        #with self.webapp_session.post(url, json=data) as resp:
        #    if resp.status != 200:
        #        raise Exception(f"Error registering DID, response code {resp.status}")
        #    print(resp)
        ...
        #r = yield from aiohttp.request('post',url,json=data,headers = headers)

        #self.webapp_session.post("http://127.0.0.1:3000/register", json=data) as resp:

        try:
            print('a1')
            temp_rec = await agent.webapp_POST(
        ...
        #async with self.webapp_session.request('POST',url, json=data) as resp:
    ...
    async def listen_webhooks(self, webhook_port):
        ...
        log_msg('listen|'+str(self.webhook_site)+str(webhook_port))
        await self.webhook_site.start()

    async def listen_myapi(self, myapi_port):
        self.myapi_port = myapi_port
        if RUN_MODE == "pwd":
            self.myapi_url = f"http://localhost:{str(myapi_port)}/myapi"
        else:
            self.myapi_url = (
                f"http://{self.external_host}:{str(myapi_port)}/myapi"
            )

        app = web.Application()

        #app.add_routes(web.get("/myapi/", ))
        #app.add_routes([web.get('/myapi/', self._receive_myapi)])
        #app.add_routes([web.get("/myapi/", self._receive_myapi)])
        app.add_routes([web.post("/myapi/cmd/{cmd}/", self._receive_myapi_post)])
        app.add_routes([web.get("/myapi/query/", self._receive_myapi_get)])
        runner = web.AppRunner(app)
        await runner.setup()
        self.myapi_site = web.TCPSite(runner, "0.0.0.0", myapi_port)
        log_msg('listen myapi|'+str(self.myapi_site)+str(myapi_port))
        await self.myapi_site.start()

    async def _receive_webhook(self, request: ClientRequest):
        topic = request.match_info["topic"].replace("-", "_")
        payload = await request.json()
        log_msg('_receive_webhook(1) executed| topic='+str(topic)+'|payload='+str(payload))
        await self.handle_webhook(topic, payload, request.headers)
        return web.Response(status=200)

    async def _receive_myapi_post(self, request: ClientRequest):
        cmd = request.match_info["cmd"].replace("-", "_")
        payload = await request.json()
        log_msg('_receive_myapi_post | cmd=' + str(cmd) + '|payload=' + str(payload) + '|match_info=' + str(request.match_info))
        res = await self.handle_api_cmd(cmd, payload, request.headers)
        print('handle_api_cmd return value=',res)
        #json_response(data, text=None, body=None, status=200, reason=None,
        #              headers=None, content_type='application/json', dumps=json.dumps)
        #return web.Response(text=ret, status=200)

        return web.json_response(res, text=None, body=None, status=200, reason=None,
                      headers=None, content_type='application/json', dumps=json.dumps)

    async def _receive_myapi_get(self, request: ClientRequest):
        log_msg('_receive_myapi_get(1) executed| match_info='+str(request.match_info))
        #cmd = request.match_info["cmd"].replace("-", "_")
        payload = None
        #payload = await request.json()
        #log_msg('_receive_myapi_get3=' + str(cmd) +'|'+str(payload))
        #await self.handle_webhook(topic, payload, request.headers)
        return web.Response(status=200)
    ...
    async def handle_webhook(self, topic: str, payload, headers: dict):
        if topic != "webhook":  # would recurse
            ...
            if method:
                ...
                log_msg('handle_webhook(1) executed| handler=' + str(handler) + '|topic='+str(topic) + "|wallet_id="+ str(wallet_id) +"|method="+ str(method) +"|payload"+ str(payload))
                asyncio.get_event_loop().create_task(method(payload))
            else:
                log_msg('handle_webhook(2) executed| handler=' + str(handler) + "|topic=" + str(topic) + "|ident=" + str(self.ident))
                log_msg(
    ...
    async def handle_api_cmd(self, cmd: str, payload, headers: dict):
        if cmd != "cmd":  # would recurse
            handler = f"api_handle_{cmd}"
            wallet_id = headers.get("x-wallet-id")
            log_msg('handle_api_cmd|cmd='+cmd+' | handler=' + handler + " | payload=" + str(payload) + ' | headers=' + str(headers))
            method = getattr(self, handler, None)
            if method:
                EVENT_LOGGER.debug(
                    "Agent called controller myAPI POST cmd: %s%s%s%s",
                    handler,
                    f"\nPOST {self.myapi_url}/myapi/cmd/{cmd}/",
                    (f" for wallet: {wallet_id}" if wallet_id else ""),
                    (f" with payload: \n{repr_json(payload)}\n" if payload else ""),
                )
                #log_msg('handle_api_cmd1=' + str(handler) +"|"+ str(wallet_id) +"|"+ str(method) +"|"+ str(payload))
                #log_msg('handle_api_cmd2|invitaion_url=' + str(self.invitaion_url))
                if handler == 'api_handle_issuecred':
                    # add a task
                    if self.invitaion_url['invitation'] and self.invitaion_url['invitation']['recipientKeys']:
                        if len(self.invitaion_url['invitation']['recipientKeys']) >= 0:
                            self.add_task(handler, str(self.invitaion_url['invitation']['recipientKeys'][0]), payload, str(self.invitaion_url['connection_id']))
                            log_msg('New task(1)|' + str(handler) + "|" + str(self.invitaion_url['invitation']['recipientKeys'][0]) +"|"+ str(payload)+'|'+str(self.invitaion_url))
                        else:
                            self.add_task(handler, str(self.invitaion_url['invitation']['recipientKeys']), payload, str(self.invitaion_url['connection_id']))
                            log_msg('New task(2)|' + str(handler) + "|" + str(
                                self.invitaion_url['invitation']['recipientKeys']) + "|" + str(payload)+'|'+str(self.invitaion_url))
                    elif self.invitaion_url['connection_id']:
                        self.add_task(handler, str(self.invitaion_url['connection_id']), payload, str(self.invitaion_url['connection_id']))
                        log_msg('New task(3)|' + str(handler) + "|" + str(self.invitaion_url['connection_id']) + "|" + str(payload)+'|'+str(self.invitaion_url))
                    #log_msg('handle_api_cmd3='+str(len(self.tasklist)))

                    return self.invitaion_url
                else:
                    asyncio.get_event_loop().create_task(method(payload))
            else:
                log_msg('handle_api_cmd2=' + str(self.ident) +"|"+ str(handler) +"|"+ str(cmd))
                log_msg(
                    f"Error: agent {self.ident} "
                    f"has no method {handler} "
                    f"to handle myapi POST cmd {cmd}"
                )
    ...
    async def webapp_POST(
        ...
        try:
            ...
            if self.multitenant:
                ...
                self.log("POST:", path)
                self.log("Headers:", headers)
            response = await self.webapp_request(
                "POST", path, data, text, params, headers=headers
            )
            EVENT_LOGGER.debug(
                "Response from POST %s received: \n%s",
