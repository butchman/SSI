...
agentNamePrefix1 = 'iPIA'
agentNameDef1 = 'משרד הפנים'

server_defs_filename = 'server.json'

cred_list_filename = 'cred-list.json'
schema_filename_field = 'schema_filename'
cred_filename_field = 'cred_filename'

class iGenAgent(MyAgent):
    def __init__(
        self,
        ident: str,
        http_port: int,
    ...
    async def handle_connections(self, message):
        ...
        if conn_id == self.connection_id:
            ...
            if (message["state"] in ["active"] and not self._connection_ready.done()):
                self.log("Connected")
...
async def create_schema_and_cred_def(agent, revocation, schemas_list):
    with log_timer("Publish schema/cred def duration:"):
        ...
        for curr_schema in schemas_list:
            loaded_schema = None
            if path.exists(curr_schema[schema_filename_field]):
                with open(curr_schema[schema_filename_field]) as f:
                ...
                cred_rec = {
                    'cred_id': curr_schema['id'],
                    'cred_alias': curr_schema['alias'],
                    'cred_name': curr_schema['name'],
                    'cred_attributes': loaded_schema['attributes'],
                    'schema_def_id': schema_def_id,
                    'cred_def_id': credential_definition_id
                }

                #schema_list2.append((schema_def_id,credential_definition_id))
                schema_list2.append(cred_rec)
            else:
                print('Error - file '+curr_schema[schema_filename_field]+' does not exits')
...
'''
find_key|2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt|2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt
find_key-FOUND|2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt|2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt
Task record matching key found in position #0
<runners.support.myagent.atask object at 0x7f5d7ca2a6d8>
2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt
api_handle_issuecred
{'cred_name': 'covid_passport', 'cred_version': '1.0.3', 'credList': {'name': 'aa22', 'id': 'aa11', 'dob': 'aa44', 'vacType': '', 'manuf': '', 'lot': '', 'date1': '', 'date2': '', 'hmo': ''}}
'''
...
Task record matching key found in position #0
<runners.support.myagent.atask object at 0x7f5d7ca2a6d8>
id=2KDqfUyQyTUg6vXqS6qVR39P98oof7EFZT5TLkn4s1Tt
cmd=api_handle_issuecred
data={'cred_name': 'covid_passport', 'cred_version': '1.0.3', 'credList': {'name': 'aa22', 'id': 'aa11', 'dob': 'aa44', 'vacType': '', 'manuf': '', 'lot': '', 'date1': '', 'date2': '', 'hmo': ''}}
'''
...
def runTaskRecord(agent, aTaskRecord):
    if aTaskRecord:
        ...
        # [{'cred_id': 'nidsc1', 'cred_alias': 'nat-id-cred', 'cred_name': 'national ID', 'cred_attributes': ['מספר', 'שם פרטי', 'שם משפחה', 'שם אב', 'שם אם', 'שם סב', 'תאריך לידה', 'שנת לידה', 'ארץ לידה', 'מין', 'תאריך הנפקה', 'תאריך תוקף', 'timestamp'], 'schema_def_id': 'Bn7rQJgSn2nLY1xcJVaVpg:2:תעודת זהות לאומית:1.2.4', 'cred_def_id': 'Bn7rQJgSn2nLY1xcJVaVpg:3:CL:4612:משרד הפנים|תעודת זהות לאומית'}]

        if aTaskRecord.cmd == 'api_handle_issuecred':
            agent.active_schema_list
            find_cred
...
'''
        api_handle_issuecred
        {'cred_name': 'covid_passport', 'cred_version': '1.0.3',
         'credList': {'name': 'mmm', 'id': 'ooo', 'dob': '', 'vacType': '', 'manuf': '', 'lot': '', 'date1': '',
                      'date2': '', 'hmo': ''}}
        ...
        print('---------------------------------------------------------------------------------------')
        await
        agent.admin_POST("/issue-credential/send-offer", offer_request)
...
async def main(
    ...
    try:
        ...
        df_filename = server_defs_filename
        if args.server_defs:
            df_filename = args.server_defs


        if path.exists(df_filename):
            serverDefs = None
            with open(df_filename) as f:
                serverDefs = json.load(f)
        else:
            serverDefs = {}
            serverDefs['agent_name_prefix'] = agentNamePrefix
            serverDefs['agent_name_def'] = agentNameDef

        log_msg("Agent name prefix is::", serverDefs['agent_name_prefix'])
        log_msg("Agent name prefix is::", serverDefs['agent_name_def'])

        agent = iGenAgent(
            serverDefs['agent_name_def'],
            start_port,
            start_port + 1,
            serverDefs['agent_name_prefix'],
        ...
        schemalist = None
        if path.exists(cred_list_filename):
            with open(cred_list_filename) as f:
        ...
        # Create the schemas (only once per run)
        agent.active_schema_list = None
        if len(schemalist) > 0:
            agent.active_schema_list = await create_schema_and_cred_def(agent, revocation, schemalist)
        ...
        while True:
            ...
            log_msg("Connection detected, checking task record list (list size="+str(len(agent.tasklist))+")")
            log_msg("----------------------------------------------")
            ...
            if ind >= 0:
                ...
                runTaskRecord(agent, agent.tasklist.pop(ind))
                #agent._connection_ready.set_result(False)

                agent._connection_ready = None
                agent._connection_ready = asyncio.Future()
            else:
        ...
        exit('DONE')
        # check if there's a request connected to this invitation keyprint out the invite details
        ind = agent.find_key(agent.invitation_key)
        if ind >= 0:
            print('Task record matching key found in position #'+str(ind))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Runs a demo agent.")
    ...
    parser.add_argument(
        ...
        default='',
        metavar="<server-defs>",
        help="Set the agent server definitions",
    )
